...
- 新增「历史记录条数」数字输入，绑定 `config.general.historyLimit`（默认 200；0 = 停止新增记录，附帮助说明）。
- 进入设置时调 `historyCount()` 取现有条数；当输入值 `>0` 且 `< 现有条数` 时，在该项下方显示一行小字：「已有日志超过该条数，将在下次调用 AskHuman 时清理」，并提供**链接样式「立即清理」**按钮：点击调 `trimHistory(输入值)`，成功后刷新计数、隐藏提示。
...
- **手动 / 端到端**：
  ...
  - 设置 `historyLimit`：默认 200；设 0 停新增但旧记录可看；调小且超额 → 提示 + 「立即清理」即时裁剪；正常调用 `AskHuman` 自动裁剪到上限。
  - 清空（当前项目 / 全部）生效。
...
- **`historyLimit=0` 语义**：仅停止新增、保留旧记录（不可在 finish 把上限当 0 裁空）；裁剪只在 `limit>0` 时发生。
