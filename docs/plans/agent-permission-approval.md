...
| 设置入口 | Claude/Codex 各自在现有 Agent 卡内增加“权限审批”开关（Claude 默认开，Codex 默认关）；不新增第四种集成模式 |
| 生效关系 | CLI/MCP 模式与权限审批开关共同决定是否安装；关闭只卸 AskHuman PermissionRequest 条目，保留 Rule/MCP/CLI timeout；None 卸载条目但保留偏好 |
| 旧安装升级 | 已处于 CLI/MCP 的旧用户显示现有“需更新 / 更新”按钮；用户点更新或主动重设当前 mode 后按各自默认 preference 补齐/卸载 PermissionRequest Hook，Daemon 不静默迁移 |
...
- Claude 默认开启、Codex 默认关闭；新切到 CLI/MCP 时按 preference 安装/跳过。旧版已经处于 CLI/MCP 的用户即使缺省偏好解析为默认值，也不静默
...
2. Claude/Codex Agent 卡按 §5.9 显示“权限审批”开关（Claude 默认开、Codex 默认关）、安装/阻断/共存状态；Cursor/Grok/Windows
   显示不支持原因；
