...
- 有后续指令：用与现有插话相同的结构化包装，把用户原话放进 `<user_message>`，要求 Agent
  据此调整工作；不写死 MCP server、工具或命令名称。
- 无后续指令：要求 Agent 立即使用 **其 Instructions 中规定的提问工具**联系人类，
  不写 `AskHuman`、MCP server 名或具体工具名，也不得让 Agent 用普通输出代替提问。
- 文案放在 `prompts.rs` 作为单一来源，并用单元测试锁定“不出现具体产品/工具名”的约束。
...
- continuation prompt 有/无用户指令；XML 包装；不出现 AskHuman/MCP server/具体工具名。
