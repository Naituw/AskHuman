...
- 默认 CLI / MCP 提示词在既有“结束前必须询问、必须得到可以结束且没有更多任务的确认”规则后，
  要求 Agent **仅在用户明确批准结束本轮之后** 输出 `[user_confirmed_end_turn]`（提示词要求作为最终输出的独立尾行；
  未获该批准时严禁输出该标记）。该规则与 Stop 开关无关，避免切换开关时联动更新提示词。
- Stop Hook 在自然 Stop 的原始最后回复中，若**任意一行**（对该行 `trim` 后）精确等于该标记，则静默放行并剥离这些标记行；
  允许标记后仍有其它正文（Agent 常在标记后再补一句）。同行粘连、markdown 包裹（`` `marker` `` / `**marker**`）不命中。
  Agent 完全忘记标记时安全退化为再次发 Stop 卡。标记只用于去重，不能替代通过提问工具发送任何需要用户看到的报告、总结或文件。
...
- `[user_confirmed_end_turn]`：任意独立行（`line.trim() == marker`）命中；同行前后缀/markdown 包裹不命中；
  标记后仍有正文仍命中；命中后剥离标记行；Claude / Codex 直接字段与 Cursor transcript（含多 text block、长回复）均在截断前判定。
